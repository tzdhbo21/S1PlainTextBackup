...
##### 187#       发表于 2021-2-5 15:41
 ...
 本帖最后由 MonicaRuan 于 2021-2-5 15:44 编辑 
...
关于三门问题（和改版）中文互联网上也有过非常多论战，几年前我在隔壁看的一次屠版爆聊了几天，最后变成所谓的“语文问题”。但实际上还是死鸭子嘴硬，因为用计算机进行一模一样的模拟的话还是贝叶斯公式胜出，很多“死鸭子”是自己模拟了一遍才发现和所谓的语文理解没有关系。
...
其实确症是一个经典的条件概率“反直觉”问题。有不少流行病检测（尤其是无差别体检），都是要2次以上阳性数据才谈得上可靠，不管单次准确率有多高。
...
答案会让很多人吃惊。实际上当时开始真实检测的情况远远比这个还糟糕，一个人如果第一次检测是阳性，那么他有新冠的概率是远小于50% （即使核酸检测准确率达到99%）。所以才有了非常严格的检测条件。
...
##### 190#       发表于 2021-2-5 16:47
 ...
 本帖最后由 MonicaRuan 于 2021-2-5 16:59 编辑 
...
而就是这一百门的推理，当时（1990年！！）很多数学家和知名人士觉得是在侮辱数学和概率论（而这些人都是支持1/2的死鸭子党），具体的故事和段子你可以看我176楼里提到的那个视频，是麻省理工的计算机系的一门数学课（大概等于组合/离散数学引论）。

PS:关于这个视频虽然是一门基础的不能再基础的数学课，但是Tom Leighton可以把一个极其平凡的东西讲出花来，我印象最深刻的是博彩业的下注回报期望，一个极简单的，讲给初中生都能明白的数学道理（但是很“反直觉”），居然在博彩业几十年没有发现，最后被MIT的一个数学教授在80年代给break了，从此整个世界的博彩业更改了游戏规则。
